...
// // Update the interface to match profile API response
// interface UserType {
//   id: string;
//   email: string;
//   name: string | null;
...
//   // Update the useEffect to use profile API
//   useEffect(() => {
...
//         const challansResponse = await axios.get(`/api/vahanfin/echallan?rc_no=${rc_no}`);
//         if (challansResponse.data?.data) {
//           const pendingChallans = challansResponse.data.data.Pending_data || [];
//           setChallans(pendingChallans.map((challan: any) => ({
//             ...challan,
//             id: challan.challan_no,
...
//   // Update handlePayment to handle selected challans details
...
//   // Update the CreditAlertDialog component
...
//         <span className="font-medium">Selected: {selectedChallans.size}</span>
//         <button 
//           onClick={() => {
...
//             className="h-4 w-4"
//           />
//           Select {activeTab === 'online' ? 'Online' : 'Court'} All
//         </button>
//       </div>
...
//               No {activeTab === 'online' ? 'online' : 'court'} challans found
//             </div>
//           ) : (
//             filteredChallans.map(challan => (
//               <div key={challan.id} className="bg-white rounded-lg shadow-md p-4">
...
//                       <span className="font-medium">{challan.challan_no}</span>
//                       <div className="text-right">
//                         <p>Amount: ₹{challan.fine_imposed}</p>
//                         <p className="text-sm text-gray-500">Service Fee: ₹100</p>
//                         <p className="text-sm text-gray-500">GST (18%): ₹18</p>
//                         <p className="font-medium">
//                           Total: ₹{parseFloat(challan.fine_imposed) + 118}
//                         </p>
//                       </div>
//                     </div>
...
//                         Date: {new Date(challan.challan_date_time).toLocaleDateString()}
//                       </span>
//                     </div>
//                   </div>
//                 </div>
...
//             <p className="text-2xl font-bold text-blue-700">₹{user?.credits || 0}</p>
//             <button
...
//             className="w-full flex justify-between items-center py-2 border-t"
//           >
...
//             disabled={!isTermsAccepted || isLoading || selectedChallans.size === 0 }//(user ? totalAmount + (selectedChallans.size * 118) > user.credits : false)
//             className={`w-full mt-4 py-3 rounded-lg text-white font-medium ${
//               !isTermsAccepted || isLoading || selectedChallans.size === 0 //|| (user ? totalAmount + (selectedChallans.size * 118) > user.credits : false)
//                 ? 'bg-gray-400'
...
//             {isLoading 
//               ? 'Processing...' 
//               : `Pay ${selectedChallans.size} Challan${selectedChallans.size > 1 ? 's' : ''} (₹${totalAmount + (selectedChallans.size * 118)})`
//             }
//           </button>
//         </div>
//       </div>
