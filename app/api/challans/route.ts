...
//             amount_of_fine: parseFloat(challanItem.fine_imposed || 0),
//             state_code: challanItem.state_code,
//             fine_imposed: parseFloat(challanItem.fine_imposed || 0),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const processChallans = async (challanDataArray: any[], status: string) => {
      if (Array.isArray(challanDataArray) && challanDataArray.length > 0) {
        const challansToCreate = challanDataArray.map((challanItem: any) => {
          console.log(`Processing ${status} challan:`, challanItem);
          return {
            rc_no: vehicle.vrn,
    ...
    if (challanData?.data && typeof challanData.data === 'object') {
      // Process both disposed and pending data regardless of whether they're null
      await processChallans(challanData.data.Disposed_data, "disposed");
      await processChallans(challanData.data.Pending_data, "pending");

      // If both are empty/null and we have no data to process
      if ((!challanData.data.Disposed_data || challanData.data.Disposed_data.length === 0) && 
          (!challanData.data.Pending_data || challanData.data.Pending_data.length === 0)) {
        const emptyChallanInstance = [{
          rc_no: vehicle.vrn,
          user_id: userId,
          vehicle_id: vehicleId,
      ...
      // Handle case where data is completely missing
      console.log("No challan data provided");
      const emptyChallanInstance = [{
        rc_no: vehicle.vrn,
        user_id: userId,
        vehicle_id: vehicleId,
